...
def leer_pestana(nombre):
    ...
    df = df.fillna("")
    # Solo omitir filas donde TODAS las columnas clave están vacías
    col_gestion = next((c for c in df.columns if "GESTI" in c.upper()), None)
    col_luthier = next((c for c in df.columns if "LUTHIER" in c.upper()), None)
    col_escuela = next((c for c in df.columns if "ESCUELA" in c.upper() and "TIPO" not in c.upper()), None)
    mask_vacias = pd.Series([True] * len(df))
    if col_gestion: mask_vacias = mask_vacias & (df[col_gestion].str.strip() == "")
    if col_luthier: mask_vacias = mask_vacias & (df[col_luthier].str.strip() == "")
    if col_escuela: mask_vacias = mask_vacias & (df[col_escuela].str.strip() == "")
    df = df[~mask_vacias]
    df["_pestana"] = nombre
    return df
...
def crear_pagina(db_id, props):
    ...
    if r.status_code != 200:
        print(f"  ✗ Error al crear: {r.status_code} — {r.json().get('message','')}")
        return None
    return r.json()["id"]
 
def actualizar_pagina(page_id, props):
    r = requests.patch(
    ...
    if r.status_code != 200:
        print(f"  ✗ Error al actualizar: {r.status_code} — {r.json().get('message','')}")
        return False
    return True
...
def obtener_solicitudes_existentes():
    """Retorna un dict {titulo: page_id} para poder hacer upsert."""
    ...
    for item in resultados:
        ...
        if titulo:
            existentes[titulo[0]["plain_text"].strip()] = item["id"]
    return existentes
 
print("=" * 55)
print("SINCRONIZACIÓN LUTHERÍA — SHEETS → NOTION (UPSERT)")
print("=" * 55)
...
existentes     = obtener_solicitudes_existentes()   # {titulo: page_id}
...
print("\nSincronizando (upsert: crea o actualiza)...")
creadas      = 0
actualizadas = 0
errores      = 0
...
for _, row in df.iterrows():
    ...
    if titulo in existentes:
        ok = actualizar_pagina(existentes[titulo], props)
        if ok:
            actualizadas += 1
            print(f"  ↻ {titulo}")
        else:
